...
4. **Capture real data (open critical path; M5):** pick one model + one task via the decision matrix in `grandplan.md` §10.10.13.3; choose the `D` hook layer by the layerwise physics-probe procedure (§7.6.3) *before* geometry gating; log `(V,L,D,A)` with success labels, `episode_id`s, and a train/test split in the offline-harness input schema (below). Fastest known starting point: adapt the released SAFE rollout pipeline/datasets (`vla-safe/SAFE`; OpenVLA + π0-FAST rollouts with outcomes — verify tensors/licenses) to emit this contract.
