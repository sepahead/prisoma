...
- Optional Engram bridge: `crates/ncp-observer` is a read-only Neuro-Control-Protocol tap that turns an Engram/NEST session into another `(V,L,D,A)` source. It is **not on grandplan's critical path** (grandplan does not depend on Engram), is **excluded from the default cargo workspace** (it path-depends on the sibling `Paper2Brain/ncp`; build via `cargo build --manifest-path crates/ncp-observer/Cargo.toml`), and is **exploratory-only** until three gaps close (D `seq`-alignment, honest `L`, split/episode/label structure). The pure-PID stack builds and gates green with **no NCP/Engram/Zenoh dependency**. Bringing it up to the M5 bar is a self-contained task — `NCP_DEV_PROMPT.md`.
...
If you don’t have `just`: run `cargo run -p pid-sim --bin pid-sim-demo -- outputs/demo_runlog.jsonl`, then `cargo run -p pid-runlog --bin pid-runlog-replay -- --validate outputs/demo_runlog.jsonl`, then `cargo run -p pid-runlog --bin pid-runlog-replay -- outputs/demo_runlog.jsonl`. For sidecar provenance, use `cargo run -p pid-runlog --bin pid-runlog-replay -- --write-sidecars outputs/demo_runlog.jsonl` followed by `cargo run -p pid-runlog --bin pid-runlog-replay -- --verify-sidecars outputs/demo_runlog.jsonl`.
