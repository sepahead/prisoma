...
- Implemented: `crates/pid-core`, `crates/pid-python` (`pid_core_rs`), `crates/pid-runlog` (M1 JSONL schema + replay/validate/compare/summary/manifest/sidecar CLI), `crates/pid-bridge` (local Agent Bridge request/response dispatch core + JSON-RPC-shaped request conversion), `crates/pid-sim` (deterministic object sim + `Flow_gt`/bridge demos, flow verification, and action replay checks), `crates/pid-rerun` (prototype Rerun logging + validated run-log replay adapter), and the Experiment 0 runner (`just exp0`, `just exp0-bin`).
