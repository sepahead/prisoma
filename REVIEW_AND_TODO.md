...
- **`crates/ncp-observer` pinned to NCP v0.6.0** (the wire-0.6 enforcement cut: a *semantic* break — mandatory `ncp_version`, mandatory stamped `seq >= 1`, normative observation-plane seq stamping — with an *unchanged* serialization, so `CONTRACT_HASH` is still `24e8e6e31e1dec8a`; the `ncp_version` string `0.5` → `0.6`, not the hash, gates compatibility. The prior 0.5 wire was cut at v0.5.0 with proto-enum mode strings and the `2cf0763ad61e4f1c` → `24e8e6e31e1dec8a` recompute). The observer now enforces the wire contract on ingress via `ncp_core::decode_validated` (version-less/incompatible/unstamped frames dropped and counted); read-only data-plane tap, JSON wire unchanged for known values. Per-sample provenance markers (`metadata.l_source` / `d_source`) were added (tests `provenance_marks_recency_fallback_and_present_language`, extended `d_aligns_on_seq_not_recency`). `ncp-observer` remains off the critical path and excluded from the default workspace.
