...
- `crates/pid-runlog` now provides M1 groundwork (JSONL event schema, reader/writer, replay summary, hashes), `crates/pid-bridge` provides M2 event-core groundwork, and `crates/pid-sim` provides an M3 deterministic smoke sim with simulator-derived `Flow_gt`; full network bridge, physics backend, and real VLA/simulator experiment harness are still future work.
