...
- `crates/pid-runlog` now provides M1 groundwork (JSONL event schema, reader/writer, validation, replay summary, manifest/summary JSON, hashes), `crates/pid-bridge` provides M2 event-core/dispatcher/JSON-RPC request groundwork, and `crates/pid-sim` provides an M3 deterministic smoke sim with bridge demo and simulator-derived `Flow_gt` verification; full network bridge, physics backend, and real VLA/simulator experiment harness are still future work.
