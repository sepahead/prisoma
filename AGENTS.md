...
- Attribution methods (LRP, Integrated Gradients, DeepLIFT, Grad-CAM, TCAV, saliency/SmoothGrad, occlusion, SHAP-style probes) are documented as H9 companion diagnostics/baselines only; no first-class `attribution_computed` run-log event or Rerun attribution adapter exists yet.
