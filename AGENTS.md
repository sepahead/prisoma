...
- Implemented today: Rust estimators in `crates/pid-core`, PyO3 bindings in `crates/pid-python`, M1 run-log validation/replay/summary/manifest/sidecar groundwork in `crates/pid-runlog`, local Agent Bridge dispatch/JSON-RPC request/response groundwork in `crates/pid-bridge`, deterministic object-sim/`Flow_gt` bridge demo, stdio JSON-RPC bridge, flow checks, and action replay verification code in `crates/pid-sim`, and the Rust Experiment 0 runner (`just exp0` / `just exp0-bin`, or run the equivalent `cargo` commands below).
