...
| 5 | **First real VLA/task capture (open critical path)** | Not yet implemented (M5): pick model+task via `grandplan.md` §10.10.13.3; pick the `D` hook layer via the §7.6.3 physics-probe procedure; export `(V,L,D,A)` + success labels + `episode_id` + split into the harness input schema. Fastest known start: adapt the released SAFE rollout pipeline/datasets (`vla-safe/SAFE`; verify tensors/licenses) | Harness strict modes (`--require-success-labels --require-heldout-split --require-heldout-class-coverage --require-heldout-episode-disjoint`) pass on the capture | `REVIEW_AND_TODO.md` (critical path); `grandplan.md` §10.10.13, §12.5 |
