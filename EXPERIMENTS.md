...
> - **H2/H3/H4/H5 (redundancy/uniques/memorization/temporal) → registry H3/H4** (PID adds incremental value only inside its validated support envelope; bounded availability–tested-intervention-effect divergence), estimated under §6 and gated by §7.
...
The machine-readable M0 bundle under `protocols/` is deliberately **not freeze-ready**. It
preserves the non-promotable v1 scaffold and adds a reviewed, all-null typed v2 successor draft.
The latter keeps H1-A, H1-B, H2, H3, and H4 obligations separate; requires H1, H2, and exactly one
of H3/H4 within the maximum three-claim family; and records that every substantive selection
remains unfrozen. There is no registered confirmatory holdout, selected source/target dataset pair,
or reproducible fresh literature screen. Run `just research-governance` for structural integrity;
do not open a holdout or start evidentiary capture until a separately reviewed, content-bound
successor candidate genuinely passes the freeze-ready audit.
...
|---|---|---|---|
| **EC1 — provenance-complete replay** | Exp1 plus a structurally different adapter/environment | Pair-specific detection and replay performance over a finite preregistered valid/fault universe, with oracle labels, a separately estimated absolute sensitivity floor and mandatory pass for every registered fault–adapter pair, exact/tolerance replay margins, a false-positive endpoint, uncertainty, multiplicity, and comparison to a conventional script/container baseline. Distribution-average detection cannot rescue a failed pair | Typed adapter/capability and causal/temporal inventory, fault distribution, negative valid controls, decision rule, external reproduction; still open (`grandplan.md` §8.8) |
| **H1 Protocol A — paired frozen-snapshot algorithmic response** | Exp1 baseline cases; Exp3 manipulation constructions | Held-out direct prediction of the declared paired response functional, with calibration in prespecified or outer-training-prediction-defined bins, response reliability/Monte Carlo error, and a locked feature-vs-baseline contrast. Held-out observed responses never define bins. This is an algorithmic-sensitivity diagnostic, not a randomized or physical effect | Immutable clone state, pre-treatment moderator, instrumented/noninstrumented noninterference, declared random-number coupling, draw ledger, reverse-order/process controls; blocked on real capture and stochastic clone machinery (`grandplan.md` §6.3) |
| **H1 Protocol B — randomized closed-loop effect moderation** | Exp1/Exp3 randomized episodes | Overall ITT plus exactly one frozen primary effect-specific endpoint; causal calibration, prioritization, policy value/regret, and other confirmatory endpoints follow an explicit gatekeeping hierarchy under recorded assignment probabilities | Randomization/receipt/reset/censoring ledger, cluster-aware inference, full endpoint hierarchy, synthetic oracle and negative controls; blocked on capture and assignment runner (`grandplan.md` §6.3) |
| **H2 — prospective censoring-aware failure prediction** | Prespecified landmarks in Exp1/Exp2; later Exp5 transport | Exactly one frozen primary proper score—fixed-horizon log loss under a valid declared censoring construction or censoring-aware time-dependent Brier—plus mandatory calibration, actionability, subgroup, and external-replication gates. Event sensitivity, nondetection-retaining lead time, and decision utility are secondary and cannot rescue primary failure | A deterministic synthetic fixed-horizon/IPCW/alarm arithmetic reference is fixture-runnable (`just h2-reference`); the domain freeze, real prospective capture, full matched-access comparator frontier, and external/later-time validation remain blocked (`grandplan.md` §6.4) |
| **H3 — conditional incremental PID value** | Any H1/H2 dataset only after all four gates | Nested full-target-population improvement from adding preregistered PID features to the strongest valid non-PID model under the active H1/H2 endpoint | Population/measure/estimator/application gates, train-reference local construction, exact M1 fallback, and a frozen warning-code allowlist/disposition map; unknown warnings fail closed. Application gate currently blocked (`grandplan.md` §7) |
| **H4 — availability–tested-intervention-effect divergence** | Exp3 input/internal intervention pairs; Exp1 positive/negative controls | Under one frozen tuple and outcome, simultaneous held-out availability superiority and randomized cell-average effect equivalence in predeclared observable cells or training-only identified conditional-average-treatment-effect (CATE) regions; report the conservatively certified target-weighted region mass, never individual-effect prevalence | Fixed target sampling source/weights and transport assumptions, one primary layer/probe/construction/outcome, weight uncertainty, engagement/support gates, positive/negative controls, strong simultaneous confirmatory control, and joint power; a second construction is replication, and capture/intervention pilot remains blocked (`grandplan.md` H4, §6.6) |
...
claim needs a separately untouched target policy/task family. Confirmatory H4 uses a randomized
cell-average policy, execution, or physical-outcome effect in predeclared baseline-observable cells,
or in a low-complexity CATE-region rule learned entirely in discovery/training data and frozen
before an untouched randomized confirmation sample. Treatment randomization identifies effects in
the randomized sample but does not by itself define target-population weights. Freeze the target
sampling source, selection indicator, overlap and transport assumptions when needed, weight
estimator/uncertainty, and one primary tuple of \(Q\), representation site,
probe/metric/reference, intervention construction and dose, exactly one outcome, region rule, and
superiority/equivalence margins. A cell counts only if an
intersection–union test establishes both availability superiority and effect equivalence, using
simultaneous confidence bounds or another strong familywise procedure across the complete primary
cell family. The target-weighted mass is a mass of baseline regions with two population-level
properties; marginal arm laws and an ATE do not identify individual-effect prevalence. A
significant decodability result beside a nonsignificant intervention result is not evidence of
divergence. FDR is secondary-only and cannot promote a discovered region into the confirmatory
claim.
...
|---|---|---|---|---|
| 0 | M0 analysis-governance integrity | `just research-governance`; separately run both `python scripts/audit_research_governance.py --require-freeze-ready` and `python scripts/audit_research_governance_successor.py --require-freeze-ready` only as closed-gate checks | The base audits pass the honest historical v1 scaffold and all-null typed v2 successor draft. Both freeze-ready modes must fail until protocol/domain choices, typed EC1/H1/H2/H3/H4 obligations, estimands, endpoints, multiplicity, useful margins, holdout commitments, transport/contamination/rights evidence, comparator dispositions, and fresh search provenance are frozen. They check completeness/integrity, not scientific correctness. Evidentiary capture remains blocked. | `grandplan.md` §12 M0; `protocols/README.md` |
...
| **H4** availability–tested-intervention-effect divergence | Randomized closed-loop confirmation; snapshot work is pilot/replication only | Requires one frozen region/weight/probe/intervention/outcome tuple, availability superiority plus cell-average effect equivalence, target sampling/transport, engagement/support controls, joint power, and simultaneous familywise inference; it does not establish natural non-use |
...
**Safe-mode rule:** read-only Agent Bridge sessions allow contract-description, status, and replay queries. They log and reject mutating, file-writing, and lifecycle-ending requests. The in-repo stdio bridge exposes this as `pid-sim-bridge-stdio --safe-mode`; TCP/WebSocket start there automatically and require explicit `--allow-mutations`. Both network binaries refuse non-loopback bind addresses, but forwarding, proxying, or tunnelling a loopback listener is not prevented. TCP/stdio JSONL lines are capped at 1 MiB; the WebSocket HTTP upgrade is capped at 16 KiB and each incoming client frame at 1 MiB; network reads/writes time out after 30 seconds per operation. Standard profiles have no total request/session, request-count, or aggregate-traffic limit. Progress-making trickle traffic can persist. The optional Engram profile adds finite request-count, aggregate-input, and run-log limits. It has no independent wall-clock deadline.
...
# Hyperbolicity is not part of the default pid_core_rs 0.9 review namespace. It is available only
# through an explicit local experimental source build; no published wheel is claimed. The Rust
# harness records its sampled four-point summary as a descriptive diagnostic; do not invent a
# default Python call or use that statistic as scientific eligibility.
```
...
#### 4.2.1 Attribution Diagnostic Sanity Checks (Separate From PID Gates)

Attribution methods do not inherit the kNN geometry assumptions above, but they need their own
method-specific sanity and ranking-sensitivity checks before they can serve as exploratory H4
companion diagnostics or a strong baseline for H1/H3:
...
- **Deletion/occlusion or perturbation tests:** compare top-attributed removal against prespecified
  ...
  built from the mean and SEM of a few controls for one input is not calibrated evidence of causal
  or mechanistic use.
...
Exploratory attribution artifacts use the implemented first-class `attribution_logged` event:
method, target output, layer, modality, baseline, score hash, a legacy compatibility boolean named
`faithfulness_check`, and artifact URI. The active `experiments/attribution/` reference artifact is
a **deletion-ranking-sensitivity diagnostic** on a small reference model. Its current epsilon-LRP
method is a detached-attention, value-path-only baseline, explicitly not AttnLRP; gradient×input is
the second implemented method. The gate content, including predictor-determinism provenance, is
bound to its derived identifier. It uses selection-disjoint and group-disjoint cases, abstains on
every exact attribution-magnitude tie, compares mean absolute deletion sensitivity with bounded
per-case random-ranking references, and aggregates group wins with a conservative one-sided
binomial tail. Exactly one predeclared primary method can set the legacy boolean; secondary methods
remain diagnostics. That boolean means only that the declared ranking reached output-sensitive
baseline replacements sooner under the frozen design. It is not a causal-use, mechanism, or broad
attribution-faithfulness result; replacement inputs can be out of distribution and dependent
features remain order-sensitive. The complete method-plus-gate work is preflighted before
attribution. Each method publishes an exact-byte content-addressed NumPy artifact and a canonical,
reconstructable JSON evidence bundle binding model parameters, gate, cases, inputs, baselines,
relevance arrays, decision evidence, software versions, and source hashes; companion
`artifact_logged` events include both in the canonical manifest. The producer replaces the run-log
name last. The Rerun adapter surfaces the recorded compatibility check/provenance, not validated
faithfulness. Only the standalone converter's explicit `--load-attribution-artifacts` mode reads a
confined,
regular, non-symlinked NumPy v1.0 `<f8` artifact, capped at 1024 finite values, and it requires the
recorded exact SHA-256 and canonical shape to match before output. Bridge export keeps external
loading disabled. Path checks do not protect against every concurrent filesystem race, and
publication is not a cross-file transaction. Production VLA/LXT hooks and a production-model
validation study remain future work.

### 5.4 Conditional PID/CI Analysis Contract

The former per-episode sliding-window Python sketch is retired. The default 0.9 Python review
namespace does not expose continuous shared-exclusions PID, and no published wheel is claimed.
Tens of autocorrelated rows inside one episode do not constitute an estimable population.
...
### 5.6 H4 Availability–Tested-Intervention-Effect Protocol and Attribution Baselines

The confirmatory H4 branch is an intervention study, not an attribution-map benchmark. Before the
holdout:

1. freeze a target population, sample source, selection indicator, and weights over pre-assignment
   observable cells, or learn a low-complexity CATE-region rule using discovery/training outcomes
   only and lock it before independent randomized confirmation. When the randomized sample is not
   the target, also freeze overlap diagnostics, effect modifiers, conditional transport
   assumptions, target-weight estimation, and weight-uncertainty propagation;
2. freeze exactly one primary tuple: task variable, representation site, probe and availability
   metric/reference, intervention construction/dose, exactly one policy, execution, or physical
   outcome, region rule, target weights, availability superiority margin, effect-equivalence
   margin, and minimum useful target-region mass;
3. estimate held-out availability and randomized cell-average ITT effects without treating either
   arm's factual outcome as an observed individual effect;
4. certify a cell only when the intersection–union decision passes both availability superiority
   and effect equivalence and the frozen cell-level engagement, specificity, receipt, support, and
   control gates pass. Never filter or reweight the primary ITT analysis by realized
   post-assignment engagement or receipt;
5. use simultaneous bounds or another prespecified strong familywise procedure over all primary
   cells and both components, propagate target-weight uncertainty, and form a conservative lower
   bound on the target-weighted region mass;
6. pass a joint design simulation covering availability, equivalence, sampling/treatment support,
   engagement, controls, sparse cells, weight uncertainty or misspecification, and the complete
   simultaneous decision under null and alternative scenarios; and
7. reserve FDR for explicitly secondary region/layer/construction screens. A secondary discovery
   does not become confirmatory without a new untouched study.

The resulting region mass is not the prevalence of units with zero or nonzero individual effects.
The two marginal randomized arm laws and their ATE do not reveal the cross-world coupling of
potential outcomes.

For the same episodes, run a small prespecified set of attribution probes and method-specific sanity
tests if the model exposes the required gradients/layers. These are secondary companion
diagnostics. Do not promote the current reference probe's deletion-ranking-sensitivity event flag
into a production-model causal-use or mechanism claim:
...
- preregister the attribution baseline, layer, sample-to-episode aggregation, modality mapping, and
  deletion-ranking-sensitivity endpoint; require deletion/randomization controls independently of
  PID;
...
- report agreement or disagreement descriptively after both methods pass their own controls. It may
  delimit method scope, but it cannot identify causal policy use or validate either estimand.
...
**Protocol role:** an H1 Protocol A/Protocol B manipulation-response pilot or an H4
availability–tested-intervention-effect study,
...
The default `pid_core_rs` 0.9 review namespace omits continuous shared-exclusions PID, and no
published wheel is claimed. Run any continuous flow analysis through the feature-pinned Rust
offline harness with explicit per-axis population-support declarations, the exact preprocessing
hash, and separate population, measure, estimator, and application verdicts. Do not re-create
removed scalar Python calls or use the experimental migration module in an evidentiary workflow.
...
not registry H4, whose endpoint is the bounded availability–tested-intervention-effect divergence
defined in the binding table and
