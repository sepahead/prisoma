...
| 6 | Exp1–Exp5 protocols (§5–§9 of this document) | `just h1-preflight` now exercises the shared H1 structural/noninterference contract on content-addressed passing and failing fixtures. This is software plumbing only. Protocol A clone/response scoring, Protocol B assignment/effect scoring, the prospective H2 landmark/alarm pipeline, real capture, and the applicable estimator/measure gates remain blocked | The preflight must pass before either H1 protocol, but a fixture pass is not H1 evidence. Per-claim metrics + controls remain those in the binding §0.1 table; kill rules are in `grandplan.md` §3.8 and statistics in §6. No capture-size claim may be made from the current idealized simulator | `grandplan.md` §5, §6.3 |
