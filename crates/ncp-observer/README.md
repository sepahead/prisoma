...
Pinned to NCP **`v0.2.4`** (`Cargo.toml`), excluded from the default workspace.
...
`v0.2.4` pin (proto-native wire) decodes it, so this observer now ingests `#10`
...
(tag `v0.2.4`) and pulls Zenoh, so it is heavier than the pure-PID crates. The
