...
1.  **Implemented groundwork:**
    ...
    *   Deterministic object-sim smoke with simulator-derived `Flow_gt`, constant-velocity `flow_pred`, Agent Bridge stdio/TCP/WebSocket smokes, toy labeled harness, offline `(V,L,D,A)` artifact harness, and the content-addressed `pid-h1-preflight` passing/failing fixture path. The preflight is shared structural/noninterference plumbing, not Protocol A/B evidence. The bridge is partial M2; target-wide UI/VLA/backend control coverage and subscriptions are not complete.
