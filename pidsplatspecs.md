...
1.  **Implemented groundwork:**
    ...
    *   Deterministic object-sim smoke with simulator-derived `Flow_gt`, constant-velocity `flow_pred`, Agent Bridge stdio/TCP/WebSocket smokes, toy labeled harness, offline `(V,L,D,A)` artifact harness, the content-addressed schema-v2 `pid-h1-preflight` fixture path, and the exact-bound deterministic `pid-h1-protocol-a` synthetic finite-benchmark scoring reference. These H1 paths are software primitives, not real Protocol A/B or H1 evidence. The bridge is partial M2; target-wide UI/VLA/backend control coverage and subscriptions are not complete.
