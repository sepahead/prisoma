...
| :--- | :--- | :--- | :--- |
| **Run log** | `crates/pid-runlog` JSONL events + replay summary | Schema v1; M1 groundwork implemented; includes embedding/sim/bridge event types | MIT (project) |
| **Agent Bridge core** | `crates/pid-bridge` | Local request/response schema + run-log integration; network server planned | MIT (project) |
| **Deterministic sim smoke** | `crates/pid-sim` | Object-only fixed-step sim + simulator-derived `Flow_gt`; physics backend trait/Rapier remains planned | MIT (project) |
