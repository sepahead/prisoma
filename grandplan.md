...
**Version:** 10.7 (First-principles spec audit + statistics-plan slice: math/precision corrections (δ-direction, digamma, r̄ identities, CI₃ sign scope, distance-concentration conditions), hypothesis-system hardening (H7a/H7b split, H5 operationalization, H2 availability-vs-use, discrete-regime preregistration, §14.8 preregistered statistical analysis plan, §9.7.2b H2/H3 protocol, reconciled publication gates, sign-safe SSI), July-2026 literature refresh (§12.6; novelty re-verified — still no PID-on-VLA anywhere), NCP pin doc sync v0.5.2→v0.5.3. Same-day triple-check addendum: hostile-review statistical repairs to the new §14.8/§9.7.2b/H9/SSI content (endpoint units, regime multiplicity, placebo criterion, calibration circularity), pid-rs submodule → **0.4.0** (upstream tags v0.3.0/v0.4.0 cut) — **Exp0 verdict on the synthetic controls is now NO-GO** (was PIVOT; stricter bias-corrected diagnostics, not a science change). Research conclusions unchanged from v10.4)
**Date:** 2026-07-06
...
- **Current estimator status (repo-local):** see `findings.md` for the latest Exp0 summary (as of this version (pid-rs 0.4.0), Exp0 reports a **NO-GO** on the synthetic high-d controls (PIVOT under pid-rs 0.3.0; the 0.4.0 bias-corrected diagnostics are stricter); do not interpret continuous PID atoms outside a validated pipeline).
...
- **Implemented (2026-06-12):** the offline harness emits per-pair `discrete_saturation` diagnostics (unique-bin fractions for each source, target, and the joint, plus a `saturation_warning` flag at the 0.8 threshold) in discrete modes. Empirical confirmation on the checked smoke fixtures: a 7-dim continuous `A` at `n=48` yields target unique-fraction 1.0 and joint MI pinned at the `ln(48)≈3.87` ceiling — flagged invalid, exactly as this section predicts. Treat any flagged pair as non-evidence.
...
**Primary pass criteria (coherence gates; do not hard-code dimension cutoffs):**
- **No NaN/Inf** in MI / `I^sx_∩` / invariants for the settings you intend to use.
...
- **Noise-dimension invariance:** as you concatenate *independent* noise dimensions, the estimated MI/`I^sx_∩` (and any derived PID atoms) should be **statistically consistent** with the reference values computed on the signal-only system (no systematic drift beyond measured uncertainty).
...
|--------|------------|----------------|
| **Synergy Stability Index (SSI)** | `SSI := −IQR(Syn)` in nats, across the **preregistered perturbation axis** — `SSI_V` over V0→V3 at W0 is the primary; `SSI_W` (W0→W3 at V0) is secondary. Quartiles use the inclusive (Tukey) convention; per-level `Syn` estimates carry block-bootstrap CIs, reported alongside. (v10.7, final form: the original `1 − Var/Mean` was sign-unsafe, and an intermediate `1 − IQR/(\|median\|+ε)` normalization was still unbounded/unstable when median `Syn` ≈ 0 — exactly where `I^sx_∩` synergy lives. Unnormalized negative dispersion is sign-safe, bounded above by 0, and a single commensurable statistic for every task.) | Higher (closer to 0) = more stable integration across perturbation levels. An IQR over 4–5 severity levels is an order statistic of noisy estimates — treat SSI as a coarse screen, never over-read small differences. The Spearman trend of `Syn` vs severity is a *separate, exploratory* statistic (it measures monotone drift, not dispersion) and never substitutes for SSI in the H4 endpoint |
...
1. On unperturbed (V0/W0) held-out episodes, estimate `Red(V,L;A)` per **task** under the active validated regime (continuous, discrete, or CI-proxy — label the measure).
...
| Geometry gates + screening | `pid-rs/crates/pid-core` | Implemented | Intrinsic dimension, distance concentration, δ/δ_rel; hierarchy helpers |
| Supervised dim reduction (PLS) | `pid-rs/crates/pid-core` (`pls.rs`) | Implemented (2026-06-11) | NIPALS-PLS2 fit/transform; addresses the Exp0 finding that unsupervised projections fail when signal variance ≈ noise variance; leakage rules + regime re-validation required (§8.2.3) |
...
| Block-bootstrap uncertainty | `pid-rs/crates/pid-core` (`bootstrap.rs`) | Implemented (2026-06-11) | `block_bootstrap`/`block_bootstrap_paired` + `BootstrapConfig`; joint row-block resampling; variance-only (does not correct kNN bias; §8.4.3) |
...
**Claim (narrowed; falsifiable):** Under matched task difficulty and preprocessing, higher redundancy between sources (e.g., `Red(V,L;A)` or `Red(V,D;A)`) predicts **robustness to single-modality corruption**: when you ablate/corrupt one modality, actions and success degrade more gracefully than when redundancy is low.
...
Each registry hypothesis with an active protocol names **exactly one** preregistered primary endpoint; everything else it reports is exploratory and labeled as such:

| Hypothesis | Primary endpoint (statistic, unit of analysis, predicted direction) |
|---|---|
| H1 | Held-out episode-level AUROC gain of {baseline features + PID/CI features} over {baseline features alone} for failure prediction; paired episode-level block bootstrap; **predicted gain > 0** |
| H2 | Spearman ρ between pre-ablation `Red` and the single-modality success-vs-severity **slope**, **across tasks** (n up to the task inventory; task families are *clusters* handled by family-blocked bootstrap CIs — families are far too few to be the analysis unit themselves); **predicted ρ > 0** (higher `Red` ⇒ less-negative slope = gentler degradation) (§9.7.2b) |
| H3 | **Mean per-case Kendall τ** between the 2–3-element `Unq` ordering and the matched-intervention effect-size ordering, across ≥ 20 matched cases (per-case τ ∈ {−1, −1/3, +1/3, +1}; family-blocked case-resampling bootstrap); **predicted mean τ > 0** (§9.7.2b) |
| H4 | Spearman ρ between the preregistered synergy-stability statistic (SSI, §9.7.2: SSI := −IQR, higher = more stable) and L0→L2 success degradation, **across tasks** (n stated ex ante; families as clusters); **predicted ρ < 0** (more stable ⇒ less degradation) |
| H5 | Trend statistic (slope / early-vs-late contrast) of the windowed primary construct (§3.6.3 operationalization) vs composition-stage index; episode-level block bootstrap; **predicted decline in failing episodes relative to succeeding ones** |
| H7b | Difference in synergy–failure correlation between world-model-stage failures and execution-stage failures; **predicted: stronger for world-model-stage failures** |
| H8 | Agreement rate between geometry-gate verdicts and realized estimator stability on synthetic + real controls, tested against a **verdict-permutation null** (α = 0.05, agreement > chance) |
...
- Before the first real capture (M5), run a **simulation-based power analysis** on synthetic/toy-harness data with the actual grouped/episode-level bootstrap. The analysis must count the **correct analysis units for each endpoint** — episodes for H1, **tasks** for H2/H4, **matched cases** for H3 — not just episodes: adding episodes sharpens per-task estimates but never increases the number of tasks, so an episode-only power calculation is a category error for the correlational endpoints.
...
- **Preregistered minimum effect sizes:** H1 ΔAUROC ≥ 0.05; H2/H4 |ρ| ≥ 0.3 (across tasks); H3 mean per-case τ ≥ 1/3 (one concordant-pair step above chance). These are commitments — changing them after seeing data is a protocol violation.
...
- If the feasible capture cannot power a primary endpoint, say so *before* running it and either scale the capture or downgrade that hypothesis to estimation-with-CIs (no significance test) — preregistered as such.
...
|----------|-------|----------------|
| **Show-Stoppers (Cat 1)** | 4 (was 5; DreamVLA mitigated) | Could kill project entirely |
| **Major Blockers (Cat 2)** | 7 | Require significant scope pivots |
| **Minor Blockers (Cat 3)** | 8 | Workarounds exist; manageable |

**Overall Risk Assessment:** HIGH but tractable. Success depends critically on Experiment 0 outcomes. ~~DreamVLA availability~~ **MITIGATED (Jan 2026; verify):** the papers state a release and this doc references public model repos; confirm weight availability + licensing before planning experiments.
...
### 18.2.1 Experiment 0 Fails Under Feasible Preprocessing
...
- **Systematic drift under independent noise dimensions** (true quantities are invariant; drift implies estimator pathology).
...
**Go/No-Go Decision:**
- **GO:** Experiment 0 passes the coherence gates (monotonicity/CMI, `\bar{r},\bar{v}` sanity) and shows no systematic drift under independent noise dimensions on the intended preprocessing pipeline.
...
**Timeline Risk:** This gate must be cleared in Month 1-2. Failure here invalidates downstream experiments that interpret continuous kNN-based `I^sx_∩`/PID atoms; proceed only with a clearly labeled pivot pipeline (MI-only invariants / discrete PID / low-d targets).
